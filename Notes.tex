...
% \section{Integrability on \mathbb{R}}

% \subsection{The Riemann Integral}

% \begin{itemize}
%     \item \textbf{Definition 5.1} \emph{Partition, Norm, Refinement} \\
%         Let $a, b, \in \mathbb{R}$ with $a < b$.
%         \begin{enumerate}
%             \item A \emph{partition} of the interval $[a, b]$ is a set of points
%                 $P = \{x_0, x_1, \ldots, x_n\}$ such that
%                 \[
%                     a = x_0 < x_1 < \cdots < x_n = b
%                 \]
%             \item The \emph{norm} of a partition $P = \{x_0, x_1, \ldots, x_n\}$
%                 is the number
%                 \[
%                     \lVert P \rVert = \max_{1 \leq j \leq n} |x_j - x_{j-1}|
%                 \]
%             \item A \emph{refinement} of a partition $P = \{x_0, x_1, \ldots, x_n\}$
%                 is a partition $Q$ of $[a, b]$ which satisfies $Q \supseteq P$.
%                 In this case we say that $Q$ is \emph{finer} than $P$.
%         \end{enumerate}

%     \item \textbf{Definition 5.3} \emph{Riemann Sums} \\
%         Let $a, b \in \mathbb{R}$ with $a < b$, let $P = \{x_0, x_1, \ldots, x_n\}$
%         be a partition of the interval $[a, b]$,
%         set $\Delta x_j := x_j - x_{j-1}$ for $j = 1, 2, \ldots, n$
%         and suppose that $f : [a, b] \to \mathbb{R}$ is bounded.
%         \begin{enumerate}
%             \item The \emph{upper Riemman sum} of $f$ over $P$ is the number
%                 \[
%                     U(f, P) := \sum_{j=1}^n M_j(f) \Delta x_j,
%                 \]
%                 where
%                 \[
%                     M_j(f) := \sup f([x_{j-1, x_j}]) := \sup_{t \in [x_{j-1}, x_j]} f(t)
%                 \]
%             \item The \emph{lower Riemman sum} of $f$ over $P$ is the number
%                 \[
%                     L(f, P) := \sum_{j=1}^n m_j(f) \Delta x_j,
%                 \]
%                 where
%                 \[
%                     m_j(f) := \inf f([x_{j-1, x_j}]) := \inf_{t \in [x_{j-1}, x_j]} f(t)
%                 \]
%                 (Since we assumed that $f$ is bounded, the numbers $M_j(f)$ and $m_j(f)$
%                 exist and are finite.)
%         \end{enumerate}

%     \item \textbf{Remark 5.4}

%         \vdots

%     \item \textbf{Remark 5.8}

%     \item \textbf{Definition 5.9} \emph{Riemann Integrable} \\
%         Let $a, b \in \mathbb{R}$ with $a < b$.
%         A function $f : [a, b] \to \mathbb{R}$ is said to be (\emph{Riemann}) \emph{integrable}
%         on $[a, b]$ if and only if $f$ is bounded on $[a, b]$, and for every $\epsilon > 0$
%         there is a partition $P$ of $[a, b]$ such that $U(f, p) - L(f, p) < \epsilon$.

%     \item \textbf{Theorem 5.10} \\
%         Suppose that $a, b \in \mathbb{R}$ with $a, b$.
%         If $f$ is continuous on the interval $[a, b]$, then $f$ is integrable on $[a, b]$.

%     \item \textbf{Definition 5.13} \emph{Integral} \\
%         Let $a, b \in \mathbb{R}$ with $a < b$ and $f : [a, b] \to \mathbb{R}$ be bounded.
%         \begin{enumerate}
%             \item The \emph{upper integral} of $f$ on $[a, b]$ is the number
%                 \[
%                     (U) \int_a^b \,\mathrm{d}x := \inf \{U(f, P) : P \ \text{is a partition of}
%                     \ [a, b]\}
%                 \]
%             \item The \emph{lower integral} of $f$ on $[a, b]$ is the number
%                 \[
%                     (L) \int_a^b \,\mathrm{d}x := \sup \{L(f, P) : P \ \text{is a partition of}
%                     \ [a, b]\}
%                 \]
%             \item If the upper and lower integrals of $f$ on $[a, b]$ are equal,
%                 we define the \emph{integral} of $f$ on $[a, b]$ to be the common value
%                 \[
%                     \int_a^b f(x) \,\mathrm{d}x :=
%                     (U) \int_a^b f(x)\,\mathrm{d}x = (L) \int_a^b f(x)\,\mathrm{d}x
%                 \]
%         \end{enumerate}

%     \item \textbf{Remark 5.14} \\
%         If $f : [a, b] \to \mathbb{R}$ is bounded, then its upper and lower integrals exists
%         and are finite, and satisfy
%         \[
%             (L) \int_a^b f(x) \,\mathrm{d}x \leq (U) \int_a^b f(x) \,\mathrm{d}x
%         \]

%     \item \textbf{Theorem 5.15} \\
%         Let $a, b \in \mathbb{R}$ with $a < b$, and $f : [a, b] \to \mathbb{R}$ be bounded.
%         Then $f$ is integrable on $[a, b]$ if and only if
%         \[
%             (L) \int_a^b f(x) \,\mathrm{d}x = (U) \int_a^b f(x) \,\mathrm{d}x
%         \]

%     \item \textbf{Theorem 5.16} \\
%         If $f(x) = \alpha$ is constant on $[a, b]$, then
%         \[
%             \int_a^b f(x) \, \mathrm{d}x = \alpha(b-a)
%         \]
...
% \begin{itemize}
%     \item \textbf{Definition 5.17} \emph{Riemann sum} \\
%         Let $f : [a, b] \to \mathbb{R}$.
%         \begin{enumerate}
%             \item A \emph{Riemann sum} of $f$ with respect to a partition
%                 $P = \{ x_0, \ldots, x_n \}$ of $[a, b]$ generated by samples
%                 $t_j \in [x_{j-1}, x_j]$ is a sum
%                 \[
%                     \mathcal{S}(f, P, t_j) := \sum_{j=1}^n f(t_j) \Delta x_j
%                 \]

%             \item The Riemann sums of $f$ are said to \emph{converge} to $I(f)$
%                 as $\lVert P \rVert \to 0$ if and only if given
%                 $\epsilon > 0$ there is a partition $P_\epsilon$ of $[a, b]$ such that
%                 \[
%                     P = \{ x_0, \ldots, x_n \} \supseteq P_\epsilon \implies
%                     |\mathcal{S} (f, P, t_j) - I(f)| < \epsilon
%                 \]
%                 for all choices of $t_j \in [x_{j-1}, x_j], j = 1, 2, \ldots, n$.
%                 In this case we shall use the notation
%                 \[
%                     I(f) = \lim_{\lVert P \rVert \to 0}
%                     \mathcal{S} (f, P, t_j) :=
%                     \lim_{\lVert P \rVert \to 0}
%                     \sum_{j=1}^n f(t_j) \Delta x_j
%                 \]

%         \end{enumerate}

%     \item \textbf{Theorem 5.18} \\
%         Let $a, b, \in \mathbb{R}$ with $a < b$, and suppose that $f : [a, b] \to \mathbb{R}$.
%         Then $f$ is Riemann integrable on $[a, b]$ if and only if
%         \[
%             I(f) = \lim_{\lVert P \rVert \to 0}
%             \sum_{j=1}^n f(t_j) \Delta x_j
%         \]
%         exists, in which case $I(f) = \int_a^b f(x) \, \mathrm{d}x$.

%     \item \textbf{Theorem 5.19} \emph{Linear Property} \\
%         If $f, g$ are integrable on $[a, b]$ and $\alpha \in \mathbb{R}$, then
%         $f + g$ and $\alpha f$ are integrable on $[a, b]$.
%         In fact
%         \[
...
%         and
%         \[
%             \int_a^b (\alpha f(x)) \, \mathrm{d}x = \alpha \int _a^b f(x) \, \mathrm{d}x
%         \]

%     \item \textbf{Theorem 5.20} \\
%         If $f$ is integrable on $[a, b]$, then $f$ is integrable on each subinterval
%         $[c, d]$ of $[a, b]$.
%         Moreover
%         \[
%             \int_a^b f(x) \, \mathrm{d}x =
...
%     \item \textbf{Theorem 5.21} \emph{Comparison Theorem for Integrals} \\
%         If $f, g$ are integrable on $[a, b]$ and $f(x) \leq g(x) \ \forall x \in [a, b]$,
%         then
%         \[
%             \int_a^b f(x)\, \mathrm{d}x \leq \int_a^b g(x) \, \mathrm{d}x
%         \]
%         In particular, if $m \leq f(x) \leq M$ for $x \in [a, b]$, then
%         \[
%             m(b-a) \leq \int_a^b f(x) \, \mathrm{d}x \leq M(b-a)
%         \]

%     \item \textbf{Theorem 5.22} \\
%         If $f$ is (Riemann) integrable on $[a, b]$, then $|f|$ is integrable on $[a, b]$ and
%         \[
%             \lvert \int_a^b f(x)\,\mathrm{d}x \rvert \leq \int_a^b |f(x)|\,\mathrm{d}x
%         \]

%     \item \textbf{Corollary 5.23} \\
%         If $f$ and $g$ are (Riemann) integrable on $[a, b]$, then so is $fg$.

%     \item \textbf{Theorem 5.24} \emph{First Mean Value Theorem for Integrals} \\
%         Suppose that $f$ and $g$ are integrable on $[a, b]$ with
...
%         \[
%             \int_a^b f(x) g(x) \, \mathrm{d}x = c \int_a^b g(x) \, \mathrm{d}x.
%         \]
%         In particular, if $f$ is continuous on $[a, b]$, then there is an
%         $x_0 \in [a, b]$ which satisfies
%         \[
%             \int_a^b f(x) g(x) \, \mathrm{d}x = f(x_0) \int_a^b g(x)\, \mathrm{d}x
%         \]

%     \item \textbf{Theorem 5.26} \\
%         If $f$ is (Riemann) integrable on $[a, b]$, then $F(x) = \int_a^x f(t)\, \mathrm{d}t$
%         exists and is continuous on $[a, b]$.

%     \item \textbf{Theorem 5.27} \emph{Second Mean Value Theorem for Integrals} \\
%         Suppose that $f, g$ are integrable on $[a, b]$, that $g$ is nonnegative on $[a, b]$,
%         and that $m, M$ are real numbers which satisfy $m \leq \inf f([a, b])$
%         and $M \geq \sup f([a, b])$.
%         Then there is a $c \in [a, b]$ such that
%         \[
%             \int_a^b f(x) g(x)\, \mathrm{d}x =
%             m\int_a^c g(x)\, \mathrm{d}x + M\int_c^b g(x)\, \mathrm{d}x.
%         \]
%         In particular, if $f$ is also nonnegative on $[a, b]$, then there is a
%         $c \in [a, b]$ which satisfies
%         \[
%             \int_a^b f(x)g(x)\, \mathrm{d}x = M\int_c^b g(x)\, \mathrm{d}x.
%         \]
% \end{itemize}
...
%     \item \textbf{Theorem 5.28} \emph{Fundamental Theorem of Calculus} \\
%         Let $[a, b]$ be nondegenerate and suppose that $f : [a, b] \to \mathbb{R}$.
%         \begin{enumerate}
%             \item If $f$ is continuous on $[a, b]$ and $F(x) = \int_a^x f(t)\, \mathrm{d}t$,
%                 \[
%                     \frac{\mathrm{d}}{\mathrm{d}x} \int_a^x f(t)\, \mathrm{d}t := F'(x) = f(x)
%                 \]
%                 for each $x \in [a, b]$.

%             \item If $f$ is differentiable on $[a, b]$ and $f'$ is integrable on $[a, b]$, then
%                 then $F \in \mathcal{C}^1[a, b]$ and
%                 \[
%                     \int_a^x f'(t)\, \mathrm{d}t = f(x) - f(a)
%                 \]
%                 for each $x \in [a, b]$.
%         \end{enumerate}
%     \item \textbf{Remark 5.29}
%         The hypotheses of the Fundamental Theorem of Calculus cannot be relaxed.

%     \item \textbf{Theorem 5.31} \emph{Integration by Parts} \\
%         Suppose that $f, g$ are differentiable on $[a, b]$ with $f', g'$ integrable on
%         $[a, b]$.
%         Then
%         \[
%             \int_a^b f'(x)g(x)\, \mathrm{d}x =
%             f(b)g(b) - f(a)g(a) - \int_a^b f(x)g'(x)\, \mathrm{d}x
%         \]

%     \item \textbf{Theorem 5.34} \emph{Change of Variables} \\
%         Let $\phi$ be continuously differentiable on a closed, nondegenerate interval $[a, b]$.
%         If
%         \[
%             \phi' \ \text{is nonzero on} \ [a, b]
%         \]
%         and if
%         \[
%             f \ \text{is integrable on} \ [c, d] := \phi[a, b],
%         \]
%         then $f \circ \phi \cdot |\phi'|$ is integrable on $[a, b]$, and
%         \[
%             \int_c^d f(t)\, \mathrm{d}t = \int_a^b f(\phi(x)) \cdot |\phi'(x)|\, \mathrm{d}x.
%         \]
